...
const razorpay = new Razorpay({
  key_id: "rzp_test_EdJHMj2g68n6on",
  key_secret: "gx5Hail7Do4pSx026vpeOTm3",
});
...
module.exports = {
      ...
      if (!attendence) {
        ...
        res.status(200).json({
          result: attendence.map(att => {
            ...
            console.log(att.subject.subjectCode)
            res.subjectCode = att.subject.subjectCode;
            console.log("after subjectCode")
  ...
  getStudentTimetable: async (req, res, next) => {
    try {
      //console.log(req.user)
      const { department, semister, registrationNumber } = req.user;
      const student = await Student.findOne({registrationNumber:registrationNumber})
      console.log(student.timetable)
      const subjectTimetable = await Timetable.find({semister:semister});
      console.log(subjectTimetable)
      if(student.timetable.length==0){
        for(let i =0;i<subjectTimetable.length;i++){
          console.log("for me aya", subjectTimetable[i])
          student.timetable.push(subjectTimetable[i])
        }
      }
      console.log(student.timetable)
      res.status(200).json({ result: student.timetable });
    }catch(err){
  ...
  postRazorPayVerify: async (req, res) => {
  try{
        const {regNo, paymentId, semister} = req.body; //pabbly ne bheja
        const studentFees = await Fees.findOne({studentRegNo:regNo})
        const newFees={
        sem:semister,
        payId:paymentId,
        isPaid:"PAID",
  ...
  if(studentFees){
    if(!studentFees.fees[semister-1]){
      studentFees.fees.push(newFees)
    }
   ...
   console.log(studentFees);
    ...
    const newStudentFees=new Fees({
      studentRegNo:regNo,
      fees:[newFees]
    })
    await newStudentFees.save();
    return res.status(200).json({newStudentFees});
  }
  
  }catch(err){
  console.error("Error while submiting fees detail to database"+err)
}
},
  getRazorpayVerify: async (req,res)=>{
    console.log("this is req body");
  const { registrationNumber } = req.user;
  const getStudentFees = await Fees.findOne({studentRegNo:registrationNumber})
  if(getStudentFees){
    res.json({result:getStudentFees.fees})
  }else{
    return res.json({
      message: "no fees is paid"
    })
  }
  console.log(getStudentFees)

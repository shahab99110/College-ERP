...
module.exports = {
  ...
  adminLogin: async (req, res, next) => {
    try {
      ...
      console.log("credentials", registrationNumber, password);
    ...
    try {
      ...
      for(let i=0;i<fees.length;i++){
          for(let j=0;j<allStudents.length;j++){
            if(allStudents[j].registrationNumber == fees[i].studentRegNo){
              if(fees[i].fees[semister-1]){
                //console.log("fees change", allStudents[j])
                allStudents[j].isCurrentSemFeesPaid = "PAID"
              }else{
                allStudents[j].isCurrentSemFeesPaid = "NOT PAID"
              }
            }else{
              allStudents[j].isCurrentSemFeesPaid = "NOT PAID"
            }

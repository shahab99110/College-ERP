...
module.exports = {
  ...
  fetchStudents: async (req, res, next) => {
    try {
      ...
      const { department, semister, section } = req.body;
      const subjectList = await Subject.find({ department, semister });
      if (subjectList.length === 0) {
        errors.department = "No Subject found in given department";
        return res.status(404).json(errors);
      }
      const students = await Student.find({ department, semister, section });
  ...
  markAttendence: async (req, res, next) => {
    try {
      const { selectedStudents, subjectCode, department, semister, section } =
        req.body;

      const sub = await Subject.findOne({ subjectCode });

      //All Students
      const allStudents = await Student.find({ department, semister, section });
  ...
  postOTP: async (req, res, next) => {
    try {
      ...
      if (faculty) {
        ...
        await student.save();
        return res.status(200).json({ message: "Password Changed" });
      } else if (faculty.otp !== otp && student.otp !== otp) {
